        ...
        fetch("/stanice/" + data.station_id)
          ...
          .then((members) =>
            members.map((member_id) =>
              fetch("/spasioci/" + member_id)
                .then((data) => data.json())
                .then((data) => (responders[data.id] = data))
            )
          );
      });
  }, []);
      ...
      <form className="UserFormAbilities" onSubmit={onSubmit}>
        <div className="FormRow">
          ...
          <select name="member_id" onChange={onChange}>
            ...
            {Object.values(responders).map((responder) => (
        ...
        <div className="FormRow">
          ...
          <select
            name="responderAbility"
            onChange={onChange}
            value={form.responderAbility}
          >
            <option value="">Odaberi</option>
            {proba[form.responderRole].map((nesto) => (
              <option key={nesto} value={nesto}>
                {nesto}
              </option>
            ))}
          </select>
